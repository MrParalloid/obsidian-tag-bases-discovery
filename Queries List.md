...
- `this.file.links.length > 1 && file.links.filter(this.file.links.contains(value)).length == this.file.links.length` - lists all the notes that share **all** the outgoing links this note. Has to be more than 1 link in common. You can set to a higher number if you actively use links.
